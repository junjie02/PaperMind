...
Current flow:
  load_papers → cluster_papers → group_by_direction → plan_chapters
  → draft_per_group → merge_outlines → review_outline ⟲ (loop if rejected)
...
def build_graph(config: Config, args: Namespace):
    ...
    workflow.add_node("plan_chapters", nodes["plan_chapters"])
